function [L, MAT_file] = cross_context_corr_twogroups(D, t, S, varargin)
...
if ~exist(MAT_file, 'file') || I.overwrite
    ...
    assert(length(unique(S.groups))==2);
    
    % number of orders tested
    n_orders = length(unique(S.orders));
    orders_per_group = cell(1, 2);
    n_orders_per_group = nan(1, 2);
    for i = 1:2
        orders_per_group{i} = unique(S.orders(S.groups==i));
        n_orders_per_group(i) = length(orders_per_group{i});
    end
    ...
    if make_samedur_comparisons
        samedur_order_pairs = [];
        for o1 = 1:n_orders_per_group(1)
            for o2 = 1:n_orders_per_group(2)
                if orders_per_group{1}(o1) ~= orders_per_group{2}(o2)
                    new_pair = [o1, o2]';
    ...
    for i = 1:n_seg_durs
        ...
        seg_start_time_in_scramstim{i} = cell(1, 2);
        for g = 1:2
            seg_start_time_in_scramstim{i}{g} = nan(n_segs_per_scramstim(i), n_orders_per_group(g));
            for l = 1:n_orders_per_group(g)
                xi = S.segs == L.unique_segs(i) ...
                    & S.orders == orders_per_group{g}(l) ...
                    & S.groups == g;
                assert(sum(xi)==1);
                for k = 1:n_segs_per_scramstim(i) % loop through all segments
                    ...
                    seg_start_time_in_scramstim{i}{g}(k,l) = (seg_index_in_scramstim-1)*seg_dur;
                end
                clear xi;
            end
        end
        ...
        if ~isempty(I.excludewin)
            short_seg_in_win = cell(1, 2);
            for g = 1:2
                short_seg_in_win{g} = false(n_segs_per_scramstim(i), n_orders_per_group(g));
                for l = 1:n_orders_per_group(g)
                    seg_on = seg_start_time_in_scramstim{i}{g}(:,l);
                    ...
                    short_seg_in_win{g}(xi,l) = true;
                end
            end
            short_seg_in_win = any(short_seg_in_win{2},2) | any(short_seg_in_win{2},2);
        ...
        if make_diffdur_comparisons 
            ...
            if n_longer_seg_durs(i)>0
                ...
                embedded_seg_start_time_in_scramstim{i} = cell(1, 2);
                ...
                for j = 1:n_longer_seg_durs(i)
                    ...
                    embedded_seg_start_time_in_scramstim{i}{g} = {...
                        nan(n_segs_per_scramstim(i), n_orders_per_group(1), n_longer_seg_durs(i)), ...
                        nan(n_segs_per_scramstim(i), n_orders_per_group(2), n_longer_seg_durs(i))};
                            ...
                            if diffdur_valid_segs{i}(k, j) && ~isempty(I.excludewin)
                                ...
                                for g = 1:2
                                    for l = 1:n_orders_per_group(g)
                                        ...
                                        xi = S.segs == L.unique_segs(longer_seg_dur_inds{i}(j)) ...
                                            & S.orders == orders_per_group{g}(l) ...
                                            & S.groups == g;
                                        assert(sum(xi)==1);
                            ...
                            if diffdur_valid_segs{i}(k, j)
                                ...
                                for g = 1:2
                                    for l = 1:n_orders_per_group(g)
                                        ...
                                        embedded_seg_start_time_in_scramstim{i}{g}(k,l,j) = longer_seg_start_time + seg_onset_relative_to_longer_seg;
    ...
    for q = 1:n_channels % analysis is done separately for every channel to save memory
        ...
        for i = 1:n_seg_durs
            ...
            Y_seg = {...
                nan(n_segs_per_scramstim(i), n_lags, n_orders_per_group(1), n_reps), ...
                nan(n_segs_per_scramstim(i), n_lags, n_orders_per_group(2), n_reps)};
            for g = 1:2
                for l = 1:n_orders_per_group(g)
                    xi = S.segs == L.unique_segs(i) ...
                        & S.orders == orders_per_group{g}(l) ...
                    ...
                    for k = 1:n_segs_per_scramstim(i) % loop through all segments
                        if ((make_samedur_comparisons && samedur_valid_segs{i}(k)) || ...
                            ...
                            targ_times = L.lag_t' + seg_start_time_in_scramstim{i}{g}(k,l);
                            ti = targ_times > (t(1)-1e-6) & targ_times < (t(end)+1e-6);
                            Y_seg{g}(k, ti, l, :) = interp1( t, X, targ_times(ti) );
                        end
                    end
                    clear ti targ_times;
                end
            ...
            if make_diffdur_comparisons
                Y_embed_seg = {...
                    nan(n_segs_per_scramstim(i), n_lags, n_orders_per_group(1), n_reps, n_longer_seg_durs(i)), ...
                    nan(n_segs_per_scramstim(i), n_lags, n_orders_per_group(2), n_reps, n_longer_seg_durs(i)), ...
                    };
                for g = 1:2
                    for j = 1:n_longer_seg_durs(i)
                        for l = 1:n_orders_per_group(g)
                            ...
                            for k = 1:n_segs_per_scramstim(i)
                                if diffdur_valid_segs{i}(k, j)
                                    targ_times = L.lag_t' + embedded_seg_start_time_in_scramstim{i}{g}(k,l,j);
                                    ti = targ_times>(t(1)-1e-6) & targ_times < (targ_times+1e-6);
                                    Y_embed_seg{g}(k, ti, l, :, j) = interp1( t, X, targ_times(ti), 'linear' );
                                end
                            end
            ...
            [L.diff_context(:,i,q), L.same_context(:,i,q), ...
                ...
                cross_context_corr_twogroups_helper(Y_seg, Y_embed_seg, ...
